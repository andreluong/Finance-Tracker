...
const getMonthlyTransactionData = async (req, res) => {
    ...
    try {
        const data = await database.overview.getMonthlyTransactionData(req.auth.userId, year);
...
const getSummary = async (req, res) => {
    ...
    try {
        ...
        const topSpendingCategories = await database.overview.getTopSpendingCategories(req.auth.userId, month, year);

        // Get frequent spending categories
        const frequentSpendingCategories = await database.overview.getFrequentSpendingCategories(req.auth.userId, month, year);

        res.status(200).json({finances, topSpendingCategories, frequentSpendingCategories});
    } catch (error) {
        console.error(error.message);
        res.status(500).json({message: "Something went wrong with getting financial summary"});
    }
}

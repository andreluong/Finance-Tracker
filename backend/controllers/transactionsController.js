...
const createTransaction = ((req, res) => {
    ...
    try {
        transactionsService.createTransaction(name, amount, description, type, category, date, req.auth.userId);
...
const getTenRecentTransactions = (async (req, res) => {
    try {
        ...
        console.error(error.message);
        res.status(500).json({error: "Something went wrong with getting recent transactions"});
    }
});
...
const getAllTransactions = (async (req, res) => {
    try {
        ...
        console.error(error.message);
        res.status(500).json({error: "Something went wrong with getting all transactions"});
    }
});
...
const updateTransaction = (async (req, res) => {
    ...
    try {
        ...
        console.error(error.message);
        res.status(500).json({error: "Something went wrong with updating transaction"});
    }
});
...
const getCategoryStats = (async (req, res) => {
    ...
    try {
        ...
        if (type === 'all' || type === 'income') {
            incomeTotal = await database.transaction.getSumOfAmountsDynamically(userId, 'income', period);
        }
        if (type === 'all' || type === 'expense') {
            expenseTotal = await database.transaction.getSumOfAmountsDynamically(userId, 'expense', period);
        }

        const categoryStats = await database.category.getTotalAmountPerCategoryDynamically(userId, type, period);
        ...
        console.error(error.message);
        res.status(500).json({error: "Something went wrong with getting category stats"});
    }
});
...
const getYears = (async (req, res) => {
    try {
        ...
        console.error(error.message);
        res.status(500).json({error: "Something went wrong with getting transaction years"});
    }
});
...
const deleteTransaction = (async (req, res) => {
    try {
        ...
        console.error(error.message);
        res.status(500).json({error: "Something went wrong with deleting transaction"});
    }
});
